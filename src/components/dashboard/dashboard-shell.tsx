...
export function DashboardShell() {
 ...
 <p className="text-sm text-muted-foreground mt-0.5">Bento grid layout · add/remove widgets from the library</p>
  ...
  <Button variant="default" size="sm" onClick={() => setSidebarOpen(true)} className="h-8 gap-1.5 text-xs shadow-sm">
  Widgets
  ...
  {visibleLayouts.map((item) => (
                <div
                  key={item.i}
                  className={`relative min-w-0 bg-card rounded-2xl shadow-sm ring-1 ring-border group ${colSpan(item.w)}`}
