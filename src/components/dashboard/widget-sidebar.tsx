...
export function WidgetSidebar({ open, onOpenChange }: WidgetSidebarProps) {
 ...
 className="w-full h-8 pl-8 pr-3 text-xs rounded-lg border border-border bg-muted/50 focus:outline-none focus:ring-1 focus:ring-ring focus:bg-card placeholder:text-muted-foreground transition-all"
 />
 </div>
 </div>
 ...
 <LayoutDashboard className="w-8 h-8 text-muted-foreground/20 mb-2" />
 ...
 {active && (
 <span className="text-[9px] px-1.5 py-0.5 rounded font-medium bg-success/10 text-success shrink-0">
 Added
 </span>
 )}
 </div>
 ...
 className="h-7 px-2.5 rounded-lg text-[10px] font-medium bg-destructive/10 text-destructive hover:bg-destructive/15 transition-all flex items-center gap-1"
 >
 Remove
 </button>
 ) : (
 <button
 onClick={() => handleToggle(w)}
 className="h-7 px-2.5 rounded-lg text-[10px] font-medium bg-primary text-primary-foreground hover:bg-primary/80 transition-all flex items-center gap-1"
 >
 Use
 </button>
 )}
