...
export function VideoBlock({ url, caption, onChange, onDelete }: VideoBlockProps) {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      onChange({ url: json.data?.url || json.url })
    } catch {
    ...
    return (
      <div className="group relative">
        <video
          src={url}
          controls
          className="w-full rounded-lg border border-border/20 max-h-96"
        />
