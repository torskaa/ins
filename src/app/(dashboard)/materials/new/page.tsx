...
type Section = "general" | "inventory" | "procurement" | "pricing" | "dimensions" | "tags"
...
export default function NewMaterialPage() {
 ...
 general: false, inventory: false, procurement: false,
 pricing: false, dimensions: false, tags: false})
 ...
 <Field id="name" label="Material Name" required>
 <Input id="name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Aluminum Sheet 2mm" required className="text-base" />
 </Field>
 <Field id="sku" label="SKU" required>
 <div className="relative">
 <Input id="sku" value={form.sku} onChange={(e) => handleSkuChange(e.target.value)} placeholder="e.g. AL-SHT-2MM" required
 ...
 <Textarea id="description" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Describe the material — specifications, grade, color, etc." rows={3} />
 </Field>

 <Field id="externalId" label={
 <span className="flex items-center gap-1.5">External ID <span className="text-[10px] text-muted-foreground font-normal">(Shopee SKU, Accounting code, etc.)</span></span>
 }>
 ...
 {imagePreview && (
 <Button type="button" variant="ghost" size="sm" onClick={removeImage} className="h-7 text-xs text-destructive gap-1">
 Remove
 </Button>
 )}
 </div>
 ...
 {/* Inventory & UoM */}
 <Card>
 <CardHeader className="pb-3"><SectionHeader title="Inventory & Unit of Measure" section="inventory" /></CardHeader>
 {!collapsed.inventory && (
 <CardContent className="space-y-4 pt-0">
 ...
 <Field id="reorderPoint" label={<span className="flex items-center gap-1">Reorder Point <span className="inline-flex items-center justify-center w-3.5 h-3.5 rounded-full bg-muted text-[9px] font-semibold text-muted-foreground cursor-help" title="Min Stock + Safety Stock (Lead Time not included yet)">⚡</span></span>}>
 ...
 {/* Procurement */}
 <Card>
 <CardHeader className="pb-3"><SectionHeader title="Procurement & Pricing" section="procurement" /></CardHeader>
 {!collapsed.procurement && (
 <CardContent className="space-y-4 pt-0">
 <div className="grid grid-cols-3 gap-4">
 <Field id="supplierId" label="Primary Supplier">
 <Select id="supplierId" options={suppliers.map(s => ({ value: s.id, label: s.name }))} placeholder="Select primary supplier" value={form.supplierId} onChange={(e: any) => setForm({ ...form, supplierId: e.target.value })} />
 </Field>
 ...
 {/* Alternative Suppliers / Supplier Pricing */}
 ...
 {/* Pricing & Tax */}
 <Card>
 <CardHeader className="pb-3"><SectionHeader title="Cost & Tax Configuration" section="pricing" /></CardHeader>
 {!collapsed.pricing && (
 <CardContent className="space-y-4 pt-0">
 <div className="grid grid-cols-4 gap-4">
 ...
 {/* Dimensions & Weight */}
 <Card>
 <CardHeader className="pb-3"><SectionHeader title="Physical Dimensions & Weight" section="dimensions" /></CardHeader>
 {!collapsed.dimensions && (
 <CardContent className="space-y-4 pt-0">
 <div className="grid grid-cols-3 gap-4">
 <Field id="weight" label={
 <span className="flex items-center gap-1">Weight (kg) <span className="inline-flex items-center justify-center w-3.5 h-3.5 rounded-full bg-muted text-[9px] font-semibold text-muted-foreground cursor-help" title="Used for warehouse space utilization">?</span></span>
 }>
